...
system_template3 = """You are an intelligent AI assisstant. Your task at hand is {task_name}. You are given three inputs: 
    input text: {input_text} 
    target text: {target_text}
    generated text: {generation}
There are two criteria for hallucination:
    ...
    If the generated text contains additional information that cannot be supported by the input text or the target text, it is a hallucination.
Otherwise, the generated text is not a hallucination.
A {annotator} evaluated the generated text looking at the input text and the target text. The {annotator} decided that the generated text is "{llm_answer}".
Here is the reasoning the {annotator} provided: {reasoning}
This is the probability assigned by the {annotator}: {probability}
"""

query_3_template = """I would like you to reevaluate inputs taking the reasoning and the probability score provided by the annotator into account.
...
llm_answers2 = read_json("round2_answers_from_GPT3-5-turbo/post_processed_test_answers_gpt-3.5-turbo-1106.json")
...
for i, llm_ans, llm_ans2 in zip(data, llm_answers, llm_answers2):
    ...
    system = system_template3.format(annotator=annotator, 
                                     task_name=task_name, 
...
with open('prompts/round3_prompts_validation_model_agnostic.json', 'w', encoding='utf-8') as f:
